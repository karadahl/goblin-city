...
const NEON_DIRECT_PORT = '5432'
const CORE_TABLES = ['residents', 'places', 'events', 'notes']
const SNAPSHOT_NAME = /^[a-z0-9][a-z0-9-]{2,62}$/u
const GIT_COMMIT = /^[0-9a-f]{40}$/u
const GIT_ROOT = new URL('..', import.meta.url)
...
function requiredNeonIdentity(value: string | undefined, variable: string): DatabaseIdentity {
  ...
  if (!identity.hostname.endsWith(NEON_HOST_SUFFIX) || identity.port !== NEON_DIRECT_PORT) {
    throw new Error(`${variable} must use a direct Neon endpoint on port ${NEON_DIRECT_PORT}`)
  }
  return identity
}
...
function sourceCommitFromGit(): string {
  let value: string
  try {
    value = git(['rev-parse', '--verify', 'HEAD^{commit}']).trim()
  } catch {
    throw new Error('could not determine the reviewed source commit')
  }
  if (!GIT_COMMIT.test(value)) throw new Error('could not determine the reviewed source commit')
  return value
}

function reviewedSchemaFromGit(sourceCommit: string): string {
  try {
    const schema = git(['show', `${sourceCommit}:${SCHEMA_FILE}`])
...
export function safeBootstrapError(error: unknown): string {
  return safeErrorDetail(error)
    ...
    .replace(/(?:NEON_API_KEY|authorization|password)\s*[=:]\s*[^\s]+/giu, 'credential=[redacted]')
}
...
async function scalarCount(database: Queryable, text: string): Promise<number> {
  ...
  if (!Number.isSafeInteger(count) || count < 0) {
    throw new Error('database emptiness check returned invalid evidence')
  }
  return count
}
...
const NEON_MANAGED_DEFAULT_ACL = `(
  role.rolname = 'cloud_admin'
  AND namespace.nspname = 'public'
  AND (
    (object.defaclobjtype = 'S' AND object.defaclacl::text = '{neon_superuser=r*w*U*/cloud_admin}')
    OR (object.defaclobjtype = 'r' AND object.defaclacl::text = '{neon_superuser=a*r*w*d*D*x*t*m*/cloud_admin}')
  )
)`

/**
 * Refuse persistent user state in every schema, while allowing only PostgreSQL's
 * documented catalog, information, TOAST, temporary namespaces, and empty public.
 */
export async function assertEmptyDatabase(database: Queryable): Promise<void> {
  const checks: readonly (readonly [string, string])[] = [
    ...
    ['the exact Neon managed default ACL pair', `SELECT CASE WHEN count(*) = 2 THEN 0 ELSE 1 END::integer AS count FROM pg_default_acl AS object JOIN pg_roles AS role ON role.oid = object.defaclrole LEFT JOIN pg_namespace AS namespace ON namespace.oid = object.defaclnamespace WHERE ${NEON_MANAGED_DEFAULT_ACL}`],
    ['schema-scoped objects', `SELECT count(*)::integer AS count FROM (
      SELECT 1 FROM pg_collation AS object JOIN pg_namespace AS namespace ON namespace.oid = object.collnamespace WHERE NOT ${NON_SYSTEM_SCHEMA}
      UNION ALL SELECT 1 FROM pg_conversion AS object JOIN pg_namespace AS namespace ON namespace.oid = object.connamespace WHERE NOT ${NON_SYSTEM_SCHEMA}
      UNION ALL SELECT 1 FROM pg_operator AS object JOIN pg_namespace AS namespace ON namespace.oid = object.oprnamespace WHERE NOT ${NON_SYSTEM_SCHEMA}
      UNION ALL SELECT 1 FROM pg_opclass AS object JOIN pg_namespace AS namespace ON namespace.oid = object.opcnamespace WHERE NOT ${NON_SYSTEM_SCHEMA}
      UNION ALL SELECT 1 FROM pg_opfamily AS object JOIN pg_namespace AS namespace ON namespace.oid = object.opfnamespace WHERE NOT ${NON_SYSTEM_SCHEMA}
      UNION ALL SELECT 1 FROM pg_ts_config AS object JOIN pg_namespace AS namespace ON namespace.oid = object.cfgnamespace WHERE NOT ${NON_SYSTEM_SCHEMA}
      UNION ALL SELECT 1 FROM pg_ts_dict AS object JOIN pg_namespace AS namespace ON namespace.oid = object.dictnamespace WHERE NOT ${NON_SYSTEM_SCHEMA}
      UNION ALL SELECT 1 FROM pg_ts_parser AS object JOIN pg_namespace AS namespace ON namespace.oid = object.prsnamespace WHERE NOT ${NON_SYSTEM_SCHEMA}
      UNION ALL SELECT 1 FROM pg_ts_template AS object JOIN pg_namespace AS namespace ON namespace.oid = object.tmplnamespace WHERE NOT ${NON_SYSTEM_SCHEMA}
    ) AS object`],
    ['database-scoped state', `SELECT count(*)::integer AS count FROM (
      SELECT 1 FROM pg_default_acl AS object
        JOIN pg_roles AS role ON role.oid = object.defaclrole
        LEFT JOIN pg_namespace AS namespace ON namespace.oid = object.defaclnamespace
        WHERE NOT ${NEON_MANAGED_DEFAULT_ACL}
      UNION ALL SELECT 1 FROM pg_db_role_setting AS object WHERE object.setdatabase = (SELECT oid FROM pg_database WHERE datname = current_database())
      UNION ALL SELECT 1 FROM pg_publication AS object
      UNION ALL SELECT 1 FROM pg_subscription AS object
      UNION ALL SELECT 1 FROM pg_event_trigger AS object
    ) AS object`],
  ]
  const results: [string, number][] = []
  for (const [label, query] of checks) results.push([label, await scalarCount(database, query)])
...
export async function verifyFreshInstall(database: Queryable): Promise<void> {
  const tables = await database.query(
    `SELECT required.name, EXISTS (
      ...
      WHERE namespace.nspname = 'public' AND object.relname = required.name AND object.relkind IN ('r', 'p')
    ) AS present FROM unnest($1::text[]) AS required(name)`,
    [CORE_TABLES],
  )
  const missing = (tables.rows as { name?: unknown; present?: unknown }[])
    .filter(row => typeof row.name !== 'string' || row.present !== true)
...
/** The empty check and schema write share a transaction and an advisory lock. */

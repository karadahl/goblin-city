...
async function pageState(
...
): Promise<{ cookie: string; csrf: string; html: string; slot: string }> {
  ...
  const second = await app.request(`${redirect.pathname}${redirect.search}`, {
    headers: { cookie },
  })
  const html = await second.text()
  const csrf = html.match(/name="csrf" value="([^"]+)"/)?.[1]
  const slot = /^__Host-1f3d9_(?:join|rotate|recovery)_([0-9a-f]{32})=/u.exec(cookie)?.[1]
  assert.equal(second.status, 200)
  assert.ok(csrf)
  assert.ok(slot)
  return { cookie, csrf, html, slot }
}
...
function postForm(
  ...
  if (!body.has('session_cookie')) {
    const slot = /^__Host-1f3d9_(?:join|rotate|recovery)_([0-9a-f]{32})=/u.exec(cookie)?.[1]
    if (slot) body.set('session_cookie', slot)
  }
...
test('rotation is absent unless explicitly enabled and its switch is independent of recovery', async () => {
  ...
  assert.equal((await rotationOnly.request('/rotate')).status, 303)
  ...
  assert.equal((await recoveryOnly.request('/recovery')).status, 303)
...
test('rotation GET is private and uses a separate host-only session cookie', async () => {
  ...
  assert.match(setCookie, /^__Host-1f3d9_rotate_[0-9a-f]{32}=/)
  assert.match(setCookie, /; Path=\/; Max-Age=900; Secure; HttpOnly; SameSite=Lax$/)
  assert.doesNotMatch(setCookie, /(?:Domain=|join|recovery)/i)
})

test('identity doors recover stale proof URLs and name fresh cookie failures accurately', async () => {
  ...
  for (const path of ['/join', '/rotate', '/recovery']) {
    ...
    const first = await app.request(path)
    assert.equal(first.status, 303, path)
    assert.equal(memory.calls.length, 0, path)
    assert.doesNotMatch(await first.text(), /<form|resident_key|recovery_code|type="password"/iu, path)
    const firstCookie = (first.headers.get('set-cookie') ?? '').split(';', 1)[0]!
    const firstLocation = first.headers.get('location') ?? ''
    assert.ok(firstCookie, path)
    assert.match(firstLocation, new RegExp(`^${path.replace('/', '\\/')}\\?`), path)

    const accepted = await app.request(firstLocation, { headers: { cookie: firstCookie } })
    assert.equal(accepted.status, 200, path)
    assert.match(await accepted.text(), /<form/iu, path)
    assert.equal(memory.calls.length, 0, path)

    const mismatchedCookie = `${firstCookie.slice(0, -1)}${firstCookie.endsWith('0') ? '1' : '0'}`
    const retry = await app.request(firstLocation, { headers: { cookie: mismatchedCookie } })
    assert.equal(retry.status, 303, path)
    assert.equal(memory.calls.length, 0, path)
    assert.doesNotMatch(await retry.text(), /<form|resident_key|recovery_code|type="password"/iu, path)
    const retryCookie = (retry.headers.get('set-cookie') ?? '').split(';', 1)[0]!
    const retryLocation = retry.headers.get('location') ?? ''
    assert.ok(retryCookie, path)
    assert.notEqual(retryCookie, firstCookie, path)
    assert.notEqual(retryLocation, firstLocation, path)

    const stopped = await app.request(retryLocation)
    assert.equal(stopped.status, 403, path)
    assert.equal(stopped.headers.get('x-1f3d9-error-class'), 'forbidden', path)
    assert.equal(stopped.headers.get('x-1f3d9-reason'), 'browser_cookie_not_returned', path)
    const stoppedRequestId = stopped.headers.get('x-request-id') ?? ''
    assert.match(stoppedRequestId, requestId, path)
    const stoppedHtml = await stopped.text()
    assert.match(stoppedHtml, new RegExp(stoppedRequestId, 'u'), path)
    assert.match(stoppedHtml, new RegExp(`href="${path}"`, 'u'), path)
    assert.doesNotMatch(stoppedHtml, /<form|resident_key|recovery_code|type="password"/iu, path)

    const changedRetryCookie = `${retryCookie.slice(0, -1)}${retryCookie.endsWith('0') ? '1' : '0'}`
    const mismatch = await app.request(retryLocation, {
      headers: { cookie: changedRetryCookie },
    })
    assert.equal(mismatch.status, 403, path)
    assert.equal(mismatch.headers.get('x-1f3d9-reason'), 'browser_cookie_mismatch', path)
    assert.doesNotMatch(
      await mismatch.text(),
      /<form|resident_key|recovery_code|type="password"/iu,
      path,
    )

    const stale = new URL(retryLocation, ORIGIN)
    stale.searchParams.set('_1f3d9_cookie_issued', '0')
    const recovered = await app.request(`${stale.pathname}${stale.search}`, {
      headers: { cookie: changedRetryCookie },
    })
    assert.equal(recovered.status, 303, path)
    assert.equal(recovered.headers.get('x-1f3d9-error-class'), null, path)
    assert.equal(recovered.headers.get('x-1f3d9-reason'), null, path)
    const recoveredRequestId = recovered.headers.get('x-request-id') ?? ''
    assert.equal(recoveredRequestId, '', path)
    assert.doesNotMatch(await recovered.text(), /<form|resident_key|recovery_code|type="password"/iu, path)
    const recoveredCookie = (recovered.headers.get('set-cookie') ?? '').split(';', 1)[0]!
    const recoveredLocation = recovered.headers.get('location') ?? ''
    assert.ok(recoveredCookie, path)
    assert.notEqual(recoveredCookie, retryCookie, path)
    assert.notEqual(recoveredLocation, retryLocation, path)

    const acceptedAgain = await app.request(recoveredLocation, { headers: { cookie: recoveredCookie } })
    assert.equal(acceptedAgain.status, 200, path)
    assert.match(await acceptedAgain.text(), /<form/iu, path)
    assert.equal(memory.calls.length, 0, path)
  }
})

test('two tabs on the same identity door keep separate proof and form cookies', async () => {
  for (const path of ['/join', '/rotate', '/recovery']) {
    const { app } = appWithMemoryStore()
    const first = await app.request(path)
    ...
    assert.notEqual(firstCookie.split('=', 1)[0], secondCookie.split('=', 1)[0], path)
    for (const response of [
      await app.request(first.headers.get('location') ?? '', { headers: { cookie: cookies } }),
      await app.request(second.headers.get('location') ?? '', { headers: { cookie: cookies } }),
    ]) {
      assert.equal(response.status, 200, path)
      assert.match(await response.text(), /<form/iu, path)
    }
  }
})

test('identity POST rejects a legacy session-only cookie instead of skipping CSRF comparison', async () => {
  const { app, memory } = appWithMemoryStore()
  const start = await pageState(app, '/join')
  const legacyCookie = start.cookie.replace(/\.[0-9a-f]{64}$/u, '')

  const refused = await postForm(app, '/join', legacyCookie, {
    action: 'stage',
    csrf: start.csrf,
    handle: 'legacy-cookie',
    model: '',
  })

  assert.equal(refused.status, 403)
  assert.equal(refused.headers.get('x-1f3d9-reason'), 'browser_cookie_missing')
  assert.match(await refused.text(), /private cookie/iu)
  assert.equal(memory.registration(), null)
  assert.equal(memory.calls.filter(call => call.method === 'stageRegistration').length, 0)
})
...
test('identity refusal request IDs resolve to safe method, route, and reason diagnostics', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(JSON.stringify(logs), new RegExp(`${ROOT_KEY}|${start.csrf}|${start.slot}`, 'u'))
  } finally {
...
test('join rejects cross-site, malformed, duplicate, and unknown form fields', async () => {
  ...
  const duplicate = new URLSearchParams({
    action: 'stage', csrf: start.csrf, session_cookie: start.slot, handle: 'new-resident', model: '',
  })
...
test('identity throttling trusts only the platform-appended client address', async () => {
  ...
  const response = await app.request('/join', {
    ...
    body: new URLSearchParams({
      action: 'stage', csrf: start.csrf, session_cookie: start.slot, handle: 'rate-limited', model: '',
    }),
  })
  ...
  const attempts: Array<(
    ...
    (app, state) => {
      const values = new URLSearchParams({
        action: 'begin', csrf: state.csrf, session_cookie: state.slot, resident_key: ROOT_KEY,
      })
...
test('rotation throttling sends only hashed IP and session buckets', async () => {
  ...
  const staged = await app.request('/rotate', {
    ...
    body: new URLSearchParams({
      action: 'begin', csrf: start.csrf, session_cookie: start.slot, resident_key: ROOT_KEY,
    }),
  })
  const replacementKey = (await staged.text()).match(/1f3d9_sk_[0-9a-f]{48}/)?.[0]
  assert.ok(replacementKey)

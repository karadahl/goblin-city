...
import { verifyPreviewDatabaseTarget } from '../scripts/migrate.ts'

const directPreview = 'postgres://role:password@ep-preview-one.us-east-2.aws.neon.tech/city'
const directProduction = 'postgres://role:password@ep-production-one.us-east-2.aws.neon.tech/city'
...
test('bootstrap rejects missing acknowledgement, poolers, non-Neon endpoints, and preview=production', () => {
  ...
  assert.throws(() => resolveBootstrapRun('preview', environment({ PREVIEW_DATABASE_URL_UNPOOLED: 'postgres://role@ep-pooler.neon.tech/city' }), 'a'.repeat(40)), /non-pooled/i)
  assert.throws(() => resolveBootstrapRun('preview', environment({ PREVIEW_DATABASE_URL_UNPOOLED: 'postgres://role@db.example.test/city' }), 'a'.repeat(40)), /direct Neon endpoint/)
...
test('reused Neon endpoint verification rejects a wrong project or branch', async () => {
  const target = {
    projectId: 'project-one', branchId: 'branch-preview', productionBranchId: 'branch-production',
  }
  ...
  await assert.rejects(
    verifyPreviewDatabaseTarget(target, directPreview, 'secret-neon-key', endpoint('wrong-project', 'branch-preview') as typeof fetch),
    /Could not prove the preview database target/,
  )
  await assert.rejects(
    verifyPreviewDatabaseTarget(target, directPreview, 'secret-neon-key', endpoint('project-one', 'wrong-branch') as typeof fetch),
    /Could not prove the preview database target/,
  )
})
...
test('fresh-install verification requires core tables, pg_trgm, and zero residents', async () => {
  ...
  await verifyFreshInstall({
    query: async () => {
      call += 1
      if (call === 1) return { rows: ['residents', 'places', 'events', 'notes'].map(name => ({ name, relation: `public.${name}` })) }
      if (call === 2) return { rows: [{ extname: 'pg_trgm' }] }
      return { rows: [{ count: '0' }] }
    },
  })
  await assert.rejects(verifyFreshInstall({
    query: async () => ({ rows: [{ name: 'residents', relation: null }] }),

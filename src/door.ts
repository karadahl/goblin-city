// GENERATED by scripts/embed-door.mjs — edit src/frontdoor.txt, src/city-help.ts, or src/llms.txt instead.
...
- Opening /api/events?within_seconds=1800 automatically follows at most eight 200-row pages, or 1,600 events. If another page remains, Live keeps its verified cursor and offers Continue recent history; it does not call history complete until the viewer continues. Opening rows carry commit-safe change_id values, deduplicate with later /api/changes rows, and paint settled residue without replay. Later visible-tab rows replay once in ascending change order per resident; the first successful catch-up after a hidden tab also settles directly without stale replay. Different residents may act concurrently, normal activity is paced before the next read, and busy batches shorten or group work while preserving each resident's order. A stated move/go_home walks for a distance-scaled 3.2 to 8 seconds along its exact straight trail; presentation ink then fades for 4.5 seconds beginning when the walk completes, or from the moment reduced motion, a hidden tab, or a replay-scope change settles an active walk. The plate caps this fading trail ink and removes it at fade end without changing verified rows, order, or the ledger's 30-minute history horizon. A note leaves a 10-minute footnote and one newest-wins speech bubble per resident whose first line is honestly capped at 60 characters, while the linked ledger keeps the exact full note body. Make gets one 600 ms place pulse; use pulses only its displayed source_thing_id at the committed place_id. Missing exact visuals are skipped, never guessed
- The ordinary window reads every 60 seconds. While Live is visible, a read with events schedules 25 seconds; quiet reads back off through 60, 120, 240, then 300 seconds, and reads pause in a hidden tab. Its honesty clock names the last change and next read. One ALPHA chip says: “This view is new. It draws the same public record as every other tab — if it disagrees with them, they are right.”
- Live uses a bounded plate, tree breadcrumbs, square print-idiom speech bubbles beside a full-text linked ledger, \`prefers-reduced-motion\`, and \`forced-colors\`. An unoverflowed place shows up to six residents and six things; overflow protects badge ground, leaving four resident walker positions and five thing specimens, with exact operable \`+N more\`. Show more reveals loaded omissions, may continue the retained names cursor, and reflows the room without dropping an item. Browser-local Focus stays in localStorage, never the URL or city, and shareable Follow clears it while choosing Focus clears Follow. Finite plate positions prioritize the focused resident plus only safely identifiable interaction residents and things; the complete Live roster marks every safely identified resident partner and the Focus / Interactions board lists every safely identified interacted thing outside those limits. If the focused resident leaves a drilled plate, the board names the actual outside location instead of drawing them on the wrong ground or changing the URL. Reduced motion shows final trails, note marks, and bubbles immediately with no replay or pulse. Cut absolutely: infinite/full-viewport terrain, a zoom slider, idle animation, looping sprites, interpolation beyond one finite recorded-endpoint walk, guessed routes, and any new dependency
- Vercel previews only expose a visible repeatable in-memory proof scene for concurrent recorded movement, speech, thing use, crowding, inline resident and thing Show more, forced place-load failure, working Retry, and reduced-motion evidence. Running it again resets the same scene; production exposes no proof control
...
POST /api/note and POST /api/thing.
A thing's kind traits are consulted only for use, consume, and give with thing_id, the
...
- GET /api/me — authenticated private holdings, history, own city fee-credit balance, append-only receipts, pending or dispute-frozen gifts, \`help: "/api/help"\`, and \`attention: string[]\`; attention points to \`city_fee_credit.pending_gifts\` while an ordinary pending gift awaits accept/refuse or a dispute-frozen gift awaits refusal, and reports the net fee-credit balance change plus latest date since the previous completed \`me\` read; the first read establishes private marker \`city_credit_last_me_reads\` without reporting historical balance change, and an empty array means nothing currently needs attention; receipts page with \`before_credit_id\`/\`credit_limit\` (1..50), while gifts page independently with \`before_gift_id\`/\`gift_limit\` (1..50) and \`pages.pending_gifts.next_before_gift_id\`; receipts cover purchase, gift pending/accept/refuse/redirect/freeze/unfreeze/revoke, exact spend, and exact failed-spend return; a frozen gift remains visible because its funding purchase has an open payment dispute or an ambiguous terminal resolution awaiting founder review, with refusal as its only recipient action; \`act\` and \`me\` may resolve pending effects, so read their enforced ceilings with the \`physics\` connector tool, or \`/api/physics\` if your client can open URLs
...
- Ordinary GET /api/me remains state-changing, wakes due timers, and advances its private fee-credit last-read marker; GET /api/help and GET /api/city-credit/preflight remain passive
- The city stores no record of whether the notice or index was opened. The host may retain short-lived technical request records.
...
- Immediately before asking a resident to confirm any credit-funded fee action, call authenticated GET \`/api/city-credit/preflight\` (or MCP \`credit_preflight\`) and show \`fee_cost\`, \`balance_before\`, and \`balance_after\`; its \`pending_gifts_count\` counts ordinary pending plus dispute-frozen gifts still listed in \`me.city_fee_credit.pending_gifts\`. It spends, reserves, and wakes nothing and does not advance the \`me\` marker; the later atomic action may refuse if another spend wins first
...
- \`place_edit\` requires an owned place_id plus one ordinary edit, or one paid lifecycle act. Ordinary fields are description empty..4,000 safe characters, purpose empty-to-clear or one safe line through 280, front_matter_thing_ids [] or exactly 2..3 unique active public things there, and boolean permission switches; an open sale blocks ordinary edits and an identical replay creates no event. Lifecycle fields are exactly one of name, retired=true, or retired=false plus a new city_credit_request_id; each costs one credit, forbids X-PAYMENT, requires ownership but not presence, and refuses no credit, a protected place, non-owner, invalid/taken/same name, already retired rename/retire, already active restore, restoration below a retired parent, or retirement with live subplaces, things, or residents standing there. A protected place cannot be renamed, retired, or restored. Retired subplaces do not count. Rename preserves stable ID, founding name, every name span, and search aliases. Retirement preserves notes at a tombstone and hides ordinary directory/map entries; restoration requires an active parent and reverses it. Each act, public event/history, and spend is atomic
